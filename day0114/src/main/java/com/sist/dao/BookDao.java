...
public class BookDao {
	...
	//고객번호를 매개변수로 전달받아 해당 고객이 주문한 도서번호, 도서명, 출판사명, 구매일, 도서가격, 구매가격을 
	//반환하는 메소드를 정의
	public ArrayList<OrderBookInfo> listOrderBook(int custid){
		ArrayList<OrderBookInfo> list = new ArrayList<OrderBookInfo>();
		String sql = "select b.bookid, bookname, publisher, orderdate, price, saleprice from book b, "
				+ " customer c, orders o where b.bookid = o.bookid and c.custid = o.custid and c.custid = ?";
		try {
	...
	//출판사명을 매개변수로 전달받아 해당출판사에서 출간하는 모든 도서 목록을 반환하는 정의
	//완성하면 "1"
	...
	//도서번호를 매개변수로 전달받아 해당도서의 정보를 BookVO로 반환하는 메소드를 정의
	...
	//도서명과 출판사를 매개변수로 전달받아 해당도서의 가격을 반환하는 메소드를 정의
